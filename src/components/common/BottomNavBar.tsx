...
const BottomNavBar = () => {
  ...
  const isActive = (path: string) => {
    return location.pathname === path ? 'text-blue-500 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400';
  };
    ...
    <nav className="fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 py-3">
      <div className="container mx-auto flex justify-around items-center">
        ...
        <Link to="/graph" className={`flex flex-col items-center ${isActive('/graph')}`}>
          ...
          <span className="text-xs mt-1">운동일지</span>
        </Link>
        
        <Link to="/foodlog" className={`flex flex-col items-center ${isActive('/foodlog')}`}>
          <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" />
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" />
          </svg>
          <span className="text-xs mt-1">식단기록</span>
        </Link>
